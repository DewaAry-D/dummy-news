...
const { put } = require('@vercel/blob'); // Import SDK Vercel Blob
...
class NewsController {
    ...
    async createPost(req, res) {
        try {
            ...
            const files = req.files || {}; // Handle jika tidak ada file sama sekali

            // 1. Buat Slug
            const slug = title.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '');

            // 2. Persiapkan Container untuk URL Gambar
            // Struktur ini harus cocok dengan apa yang diharapkan Service
            const fileUrls = {
                thumbnailImage: null,
                contentImages: []
            };
            ...
            if (files['thumbnailImage'] && files['thumbnailImage'][0]) {
                const file = files['thumbnailImage'][0];
                const filename = `newspicture/thumbnail/${Date.now()}-${file.originalname}`;
                
                // Upload ke Vercel Blob
                const blob = await put(filename, file.buffer, { access: 'public' });
                
                // Simpan URL-nya
                fileUrls.thumbnailImage = blob.url;
            }

            // --- PROSES UPLOAD CONTENT IMAGES ---
            if (files['contentImages'] && files['contentImages'].length > 0) {
                // Kita gunakan Promise.all agar upload banyak gambar berjalan paralel (lebih cepat)
                const uploadPromises = files['contentImages'].map(async (file) => {
                    const filename = `newspicture/contentnews/${Date.now()}-${file.originalname}`;
                    const blob = await put(filename, file.buffer, { access: 'public' });
                    return blob.url;
                });

                // Tunggu semua upload selesai, lalu masukkan array URL ke variable
                fileUrls.contentImages = await Promise.all(uploadPromises);
            }

            // 3. Parsing contentBlocks (Penting untuk Form-Data)
            // Saat upload file (multipart/form-data), object/array sering terkonversi jadi string JSON.
            let parsedContentBlocks = contentBlocks;
            if (typeof contentBlocks === 'string') {
                try {
                    parsedContentBlocks = JSON.parse(contentBlocks);
                } catch (e) {
                    console.error("Gagal parse contentBlocks, menggunakan raw value");
                }
            }

            // 4. PANGGIL SERVICE
            // Kirim object fileUrls yang berisi string URL (bukan file mentah)
            const newNews = await this.newsService.createPost(
                { title, slug, category, authorName, status: status || 'DRAFT' },
                parsedContentBlocks,
                fileUrls 
            ...
            // Tidak perlu fs.unlink karena file tidak pernah disimpan di disk server
            
            res.status(500).json({
                success: false,
                error: 'Gagal membuat news: ' + error.message
            });
        }
    }
